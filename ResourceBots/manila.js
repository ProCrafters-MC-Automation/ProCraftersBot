...
function createUltimateBot(options) {
    // Logging in
    UltimateBot = mineflayer.createBot(options)

    UltimateBot.state = 'idle'    // idle, combat, moving
    UltimateBot.substate = 'none'

    // Loading plugins
    UltimateBot.loadPlugin(armorManager)
    UltimateBot.loadPlugin(toolPlugin)

    // Applying listeners
    UltimateBot.on('physicTick', onTick)
    UltimateBot.once('spawn', onLogin)
    UltimateBot.on('spawn', onSpawn)
    UltimateBot.on('physicTick', onTick)
    UltimateBot.on('chat', onChatMessage)

    // Loading bot components
    movement.load(UltimateBot)
    combat.load(UltimateBot)
    convention.load(UltimateBot)
    blockfinder.load(UltimateBot)
...
// Events
function onTick() {
    UltimateBot.checkForTargets()
}


//Ultimate bot functions
function goto(args) {
    var position = {}
    ...
    if (position.x[0] == '~') {
      ...
      position.x = parseInt(UltimateBot.entity.position.x.toString().split('.')[0])
      if (str.length > 1) {
          position.x += offset
      }
    ...
    if (position.y[0] == '~') {
      str = position.y
      offset = parseInt(str.replace('~', ''))
      position.y = parseInt(UltimateBot.entity.position.y.toString().split('.')[0])
      if (str.length > 1) {
          position.y += offset
      }
    ...
    if (position.z[0] == '~') {
      str = position.z
      offset = parseInt(str.replace('~', ''))
      position.z = parseInt(UltimateBot.entity.position.z.toString().split('.')[0])
      if (str.length > 1) {
          position.z += offset
      }
  ...
  function onChatMessage(username, message, rawMessage, jsonMsg) {
    ...
    if ((options.operators.length > 0 && !options.operators.includes(username))) {
      if (options.isOp) UltimateBot.chat(`/tellraw ${username} ["",{"text":"[-] ","color":"dark_red"},{"text":"You are not allowed to perform this action.","color":"red"}]`)
      else {
        if (options.loud) UltimateBot.chat('[-] You are not allowed to perform this action.')
        else UltimateBot.whisper(username, '[-] You are not allowed to perform this action.')
      }
      return
    }
    ...
    switch (args[0].toLowerCase()) {
      ...
      case 'eat':
        ...
        break;
      case 'status':
        response = [0, 'HP: ' + UltimateBot.health.toString().split('.')[0] + ' | Food: ' + UltimateBot.food.toString().split('.')[0] + ' | ' + UltimateBot.state + '(' + UltimateBot.substate + ')']
        break;
  
      case 'stop':
        UltimateBot.stopMoving()
        UltimateBot.stopMining()
        UltimateBot.clearTargets()
        break;
      ...
      case 'mine':
        ...
        if (requireargs(1)) {
          UltimateBot.mineBlock(args[1], count)
      ...
      case 'target':
        if (requireargs(1)) {
          UltimateBot.addTarget(args[1])
          response = [1, 'Target added!']
        }
        break;
  
      case 'inventory':
        const invArray = UltimateBot.inventory.items().map(a => a.name)
        response = [0, 'Current invetory state: ' + invArray.toString().split(',').join(', ') + '.']
        break;
  
      case 'drop':
        if (requireargs(1)) {
          const item = UltimateBot.inventory.items().find(item => item.name.toLowerCase().includes(args[1]))
          if (item) {
            UltimateBot.tossStack(item)
      ...
      case 'locate':
        if (requireargs(1)) {
          ...
          if (entity) {
              ...
              position = UltimateBot.locateBlock(args[1])
          }
          if (position) {
    ...
    if (options.isOp && response[0] !== -1) {
      ...
      switch (response[0]) {
        case 0: // Neutral
          UltimateBot.chat(`/tellraw ${username} ["",{"text":"[*] ","color":"light_purple"},{"text":"${response[1]}"}]`)
          break;
        case 1: // Success
          UltimateBot.chat(`/tellraw ${username} ["",{"text":"[+] ","color":"green"},{"text":"${response[1]}","color":"aqua"}]`)
          break;
        case 2: // Failure
          UltimateBot.chat(`/tellraw ${username} ["",{"text":"[-] ","color":"dark_red"},{"text":"${response[1]}","color":"red"}]`)
          break;
      }
    } else if (response[0] !== -1) {
      if (options.loud) UltimateBot.chat(response[1])
      else UltimateBot.whisper(username, response[1])
    }
  }
  ...
  function onLogin() {
    (function eatingLoop() {
      if (UltimateBot.state !== 'combat') UltimateBot.eat()
...
// Exporting modules
module.exports = {
    createUltimateBot: createUltimateBot,
}