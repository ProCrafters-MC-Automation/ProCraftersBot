...
// CONFIG PART
const offsetX = process.argv[4];
const offsetY = process.argv[5];
const offsetZ = process.argv[6];
const placementDelayMs = 1;
const ditheringOffset = 0; // 0 = off, 5 = normal, 10 = little, 2 = much
// CONFIG END
...

{
  ...
  bot.once("spawn", () => {
    ...
    bot.chat("/tp " + offsetX + " " + offsetY + 1 + " " + offsetZ);
...
(async () => {
  let img = await Jimp.read(process.argv[7]);
